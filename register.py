...
def start_service():
    ...
    if os.path.exists(os.path.join(SETTING_DIR,"Certificates", "cert.pem")) and os.path.exists(os.path.join(SETTING_DIR,"Certificates", "key.pem")):
        if os_type == 'win32':
            service = subprocess.Popen(["LoLServer.exe"])
        else:
...
tk.messagebox.showwarning("Certificates & Ports", "Certificates are automatically configured when the server is turned on. Incase you want to override them and use your own, uncheck the 'Configure Certificates' checkbox & register, from that point until the app is relaunched certificates you put back in will not be overriden, make sure to name them as the generated certifcates are named to avoid errors. Port is set to 8000 by default, to edit it use settings.json located in /Settings. If for some unknown reason you feel the need to manually type your ipv4 and override the automatic ipv4 finder function, you can type in the IPV4 entry box. Warning: A wrong ip address will prevent the server from initiating. Info: Files uploaded before the server starts (present in /temp directory) will have a silent countdown and will only broadcast once removed, however they will be available for download until deletion.")
