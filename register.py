...
tk.messagebox.showwarning("Certificates & Ports", "Certificates are automatically configured when the server is turned on. Incase you want to override them and use your own, uncheck the 'Configure Certificates' checkbox & register, from that point until the app is relaunched certificates you put back in will not be overriden, make sure to name them as the generated certifcates are named to avoid errors. Port is set to 8000 by default, to edit it use settings.json located in /Settings. Automatic IPV4 Recognition needs a one-time very tiny bit of bandwith on startup, for a truly zero bandwith experience, type your ipv4 address in its respective box. Warning: A wrong ip address will prevent the server from initiating. Info: Files uploaded before the server starts (present in /temp directory) will have a silent countdown and will only broadcast once removed, however they will be available for download until deletion.")
